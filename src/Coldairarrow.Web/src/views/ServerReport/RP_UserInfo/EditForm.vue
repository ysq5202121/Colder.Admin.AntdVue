  ...
  >
    <a-spin :spinning="loading">
      <a-form-model ref="form" :model="entity" :rules="rules" v-bind="layout">
        ...
        <a-form-model-item label="微信用户ID" prop="WeChatId">
          <a-input v-model="entity.WeChatId" autocomplete="off" />
        </a-form-model-item>
...
export default {
  ...
  methods: {
    ...
    openForm(id, title) {
      ...
      if (id) {
        this.loading = true
        this.$http.post('/ServerReport/RP_UserInfo/GetTheData', { id: id }).then(resJson => {
          this.loading = false
    ...
    handleSubmit() {
      this.$refs['form'].validate(valid => {
        ...
        this.loading = true
        this.$http.post('/ServerReport/RP_UserInfo/SaveData', this.entity).then(resJson => {
          this.loading = false
